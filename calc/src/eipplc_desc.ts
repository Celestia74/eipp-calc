...
export function getKOChance(
  ...
  if (damage[0] >= defender.maxHP()*2 && move.timesUsed === 1 && move.timesUsedWithMetronome === 1) {
  ...
  if ((move.timesUsed === 1 && move.timesUsedWithMetronome === 1) || move.isZ) {
    ...
    for (let i = 5; i <= 9; i++) {
      if (
        ...
        defender.curHP()*2 - hazards.damage
      ) {
    ...
    if (predictTotal(
      ...
      (defender.curHP() === defender.maxHP() ? defender.curHP()*2 : defender.curHP()) - hazards.damage
    ) {
